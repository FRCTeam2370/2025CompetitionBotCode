...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    driver.leftBumper().whileTrue(new RunManipulator(mManipulatorSubsystem, 0.5));
    driver.rightBumper().whileTrue(new RunManipulator(mManipulatorSubsystem, -1));

    driver.leftTrigger().whileTrue(new RunAlgaeManipulator(mManipulatorSubsystem, 0.5));
    driver.rightTrigger().whileTrue(new RunAlgaeManipulator(mManipulatorSubsystem, -0.5));

    driver.back().onTrue(new SetSwingArm(mSwingArmSubsystem, 0.176));//loading station
    ...
    driver.y().onTrue(new SetMechanismToPose(1.70, 0.34, mSwingArmSubsystem, mElevatorSubsystem));//L2
    driver.a().onTrue(new SetMechanismToPose(2.609, 0.3177, mSwingArmSubsystem, mElevatorSubsystem));//L3
    driver.x().onTrue(new SetMechanismToPose(4.9, 0.32, mSwingArmSubsystem, mElevatorSubsystem));//L4
    ...
    //driver.back().toggleOnTrue(new SetElevatorSpeed(mElevatorSubsystem));
  }
